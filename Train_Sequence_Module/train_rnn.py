...
if args.dataset == 'serval':
    ...
    arcade_test_dataset = val_dataset.ARCADEDataset(args, dataset_path=args.data_path, transform = transform_test, transform_msk= transform_test_seg, split = 'seg_val')
...
for epoch in range(settings.EPOCH):
    ...
    loss= function_rnn.train_sam(args, net, optimizer, nice_train_loader, epoch, writer, vis = args.vis)
    ...
    if epoch and epoch % args.val_freq == 0 or epoch == settings.EPOCH-1:
        if args.dataset != 'REFUGE':
            tol,  (eiou, edice), prev_iou= function_rnn.validation_sam(args, nice_test_loader, epoch, net, writer)
            error_reduction = eiou-prev_iou
            logger.info(f'Val loss: {tol}, IOU: {eiou}, Dice: {edice}, Error Reduction {error_reduction}, Prev IoU {prev_iou} || @ epoch {epoch}.')
        else:
            tol, (eiou_cup, eiou_disc, edice_cup, edice_disc) = function_rnn.validation_sam(args, nice_test_loader, epoch, net, writer)
