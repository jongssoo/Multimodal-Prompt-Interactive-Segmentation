...
def draw_box_on_tensor(tensor, x, y, w, h, value=1):
    """
    ...
    """
    # 상단 테두리
    tensor[y, x:x+w] = value
    
    # 하단 테두리
    tensor[y+h-1, x:x+w] = value
    
    # 왼쪽 테두리
    tensor[y:y+h, x] = value
    
    # 오른쪽 테두리
    tensor[y:y+h, x+w-1] = value
...
def vis_image_iter(imgs, gt_masks, pred_masks, prev_masks,save_path, reverse = False, points = None, text = None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        if points != None:
            for i in range(b):
                ...
                prev_masks[i,0,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.5
        ...
        prev_masks = add_tag(prev_masks, 'Prev')
        pred_masks = add_tag(pred_masks, 'Prediction mask,' + 'Prompt: ' + text[0])
...
def vis_image(imgs, pred, gt_masks, pred_masks, prev_masks, label ,save_path, reverse = False, points = None, text = None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        if pred is not None:
            ...
            prompt_list = ['Thinner', 'Thicker', 'Extend', 'Denoise', 'Connect']
            prev_masks = add_tag(prev_masks, 'Prev')
            pred_masks = add_tag(pred_masks, 'Prediction mask,' + 'Prompt: ' + prompt_list[preds[0].item()])
            gt_masks = add_tag(gt_masks, 'GT: ' + prompt_list[int(label[0].item())])
...
def vis_image_pred_mask(imgs, gt_masks, pred_masks, prev_masks,save_path, reverse = False, points = None, text = None, pt_labels = None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        channel_0 = pred_masks[:, 0, :, :].clone()  # 첫 번째 채널
        channel_1 = pred_masks[:, 1, :, :].clone()  # 두 번째 채널
        channel_2 = pred_masks[:, 2, :, :].clone()  # 세 번째 채널

        pred_masks = torch.stack([channel_0, channel_1, channel_2], dim=1)
        if points != None:
            #for i in range(b):
            if args.thd:
                p = np.round(points.cpu()/args.roi_size * args.out_size).to(dtype = torch.int)
            ...
            for i in range(b):
                for j in range(len(points)):
                    ...
                    # gt_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.4
                    if pt_labels[j] == 0:
                        pred_masks[i,0,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 1.0
                        pred_masks[i,1,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 0.0
                        pred_masks[i,2,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 0.0
                    else:

                        pred_masks[i,0,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 0.0
                        pred_masks[i,1,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 1.0
                        pred_masks[i,2,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 0.0
                    # prev_masks[i,1,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.1
                    # prev_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.4
                ...
                tagged_prev_masks.append(add_tag(prev_masks[i].unsqueeze(0), 'Prev, Prompt: ' + text[i]))
...
def torch_crop_with_padding(image, center, crop_size):
    """
    이미지를 주어진 센터 포인트를 기준으로 주어진 크기로 자르되, 이미지 경계를 벗어나는 경우 자동으로 패딩을 추가하는 함수
    :param image: 원본 이미지(torch.Tensor) 형태는 (B, C, H, W)
    :param center: 자를 영역의 중심 좌표 (x, y)
    :param crop_size: 자를 크기 (width, height)
    :return: 자른 이미지(torch.Tensor)
    """
    ...
    # 자를 영역이 이미지 경계를 벗어나는지 확인하고 패딩 크기 계산
    pad_left = max(0 - (center_x - crop_width // 2), 0)
    pad_top = max(0 - (center_y - crop_height // 2), 0)
    pad_right = max((center_x + crop_width // 2) - width, 0)
    pad_bottom = max((center_y + crop_height // 2) - height, 0)

    # 이미지 자르기 (배치 크기를 유지하면서 슬라이싱)
    cropped_image = image[:, :, start_y:end_y, start_x:end_x]

    # 패딩 추가 (F.pad 사용, 상단과 좌측 패딩을 각각 설정)
    padding = (pad_left, pad_right, pad_top, pad_bottom)  # 순서: (왼쪽, 오른쪽, 위쪽, 아래쪽)
    padded_image = F.pad(cropped_image, padding, mode="constant", value=0)
