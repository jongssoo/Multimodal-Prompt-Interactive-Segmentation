...
from .transformer import TwoWayTransformer, ChannelWiseAttention, Origin_TwoWayTransformer
class MaskDecoder_CLS(nn.Module):
    def __init__(
        self,
        *,
        transformer_dim: int,
        ...
        self.cls_prediction_layer = MLP(
             transformer_dim, iou_head_hidden_dim, num_classes, iou_head_depth,sigmoid_output=False)
    def forward(
        self,
        ...
        """
        masks, cls_pred = self.predict_masks(
    ...
    def predict_masks(
        ...
        #output_tokens = torch.cat([self.cls_token.weight, self.mask_tokens.weight], dim=0)
        output_tokens = torch.cat([self.cls_token.weight, self.mask_tokens.weight], dim=0)
...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type, num_classes):
        ...
        #self.rnn = RNN(transformer_dim, transformer_dim, transformer_dim)
        ...
        #edit js
        #self.channel_wise_attention = ChannelWiseAttention(mask_num=8, embedding_dim=transformer_dim)
    ...
    def forward(
        ...
        #     hidden_embeddings = self.rnn.initHidden(sparse_prompt_embeddings)
        
        # sparse_prompt_embeddings, hidden_embeddings = self.rnn(sparse_prompt_embeddings, hidden_embeddings)
        ...
        for i_batch in range(batch_len):
            if text_embedding is not None:
                mask, iou_pred = self.predict_masks(
                    ...
                    text_embedding=text_embedding[i_batch].unsqueeze(0), #edit js
                    image_pe=image_pe,
        ...
        if hq_token_only:
            return masks_hq# , hidden_embeddings
        else:
    ...
    def predict_masks(
        ...
        #mask squence
        # mask_q_list = list(dense_prompt_embeddings.queue)
        # dense_prompt_embeddings = torch.cat(mask_q_list, dim=1)
        # emd_dim = image_embeddings.shape[1]
        # padding_size = emd_dim - dense_prompt_embeddings.shape[1]
        # if padding_size != 0:
        #     dense_prompt_embeddings = F.pad(dense_prompt_embeddings, (0, 0, 0, 0, 0, padding_size))
        #dense_prompt_embeddings = self.channel_wise_attention(dense_prompt_embeddings)
        ...
        # Run the transformer
        hs, src = self.transformer(src, pos_src, sparse_prompt_embeddings, output_tokens)
        #hs, src = self.transformer(src, pos_src, tokens)
        iou_token_out = hs[:, 0, :]
        #cls_token_out = hs[:, 1, :]
        mask_tokens_out = hs[:, 1 : (1 + self.num_mask_tokens), :]
...
class TextDecoder(MaskDecoder):
    def __init__(self, model_type, num_classes):
        super().__init__(transformer_dim=256,
                        transformer=Origin_TwoWayTransformer(
                                depth=2,
                                embedding_dim=256,
                                mlp_dim=2048,
                                num_heads=8,
