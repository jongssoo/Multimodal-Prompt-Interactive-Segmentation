...
class CHASEDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {"image":image, 
               "mask": prev_mask.unsqueeze(0), "gt": gt_mask.unsqueeze(0), "p_label" : point_label, "text": text, "pt": pt, 'image_meta_dict':image_meta_dict}
...
class ServalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        origin_size = np.array(image.shape[:2])
        ...
        if self.transform:
            origin_image = self.transform(image)

        x, y, h, w = transforms.RandomCrop.get_params(origin_image, output_size=(256, 256))
        origin_gt_mask = torch.as_tensor(gt_mask) 
        origin_modify_mask = torch.as_tensor(modify_mask)
        origin_branching_mask = torch.as_tensor(branching_mask)

        image = transforms.functional.crop(origin_image, x, y, h, w)
        gt_mask = transforms.functional.crop(origin_gt_mask, x, y, h, w) 
        modify_mask = transforms.functional.crop(origin_modify_mask, x, y, h, w)
        branching_mask = transforms.functional.crop(origin_branching_mask, x, y, h, w)
        ...
        return {"image":image, 
               "mask": prev_mask.unsqueeze(0), "gt":gt_mask.unsqueeze(0) ,"text": text, "p_label" : point_label, "pt": pt ,'image_meta_dict':image_meta_dict}
...
class ARCADEDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {"image":image, 
               "mask": prev_mask.unsqueeze(0), "gt": gt_mask.unsqueeze(0), "p_label" : point_label, "pt": pt, "text": text,'image_meta_dict':image_meta_dict}