...
def vis_image(imgs, pred_masks, gt_masks, save_path, reverse = False, points = None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        if points != None:
            for i in range(b):
                ...
                gt_masks[i,0,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.5
                gt_masks[i,1,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.1
                gt_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.4
    ...
    return
'''
def vis_image_pred_mask(imgs, gt_masks, pred_masks, prev_masks,save_path, reverse = False, points = None, text = None, pt_labels = None):
    
    b,c,h,w = gt_masks.size()
    dev = gt_masks.get_device()
    #row_num = min(b, 4)
    row_num = 4
    ...
    if c == 2: # for REFUGE multi mask output
        pred_disc, pred_cup = pred_masks[:,0,:,:].unsqueeze(1).expand(b,3,h,w), pred_masks[:,1,:,:].unsqueeze(1).expand(b,3,h,w)
        gt_disc, gt_cup = gt_masks[:,0,:,:].unsqueeze(1).expand(b,3,h,w), gt_masks[:,1,:,:].unsqueeze(1).expand(b,3,h,w)
        tup = (imgs[:row_num,:,:,:], gt_disc[:row_num,:,:,:], gt_cup[:row_num,:,:,:])
        compose = torch.cat(tup, 0)
        vutils.save_image(compose, fp = save_path, nrow = row_num, padding = 10)
    elif c > 2: # for multi-class segmentation > 2 classes
        gts = []
        ...
        for i in range(0, c):
            ...
            gt = gt_masks[:,i,:,:].unsqueeze(1).expand(b,3,h,w)
            gts.append(gt)
        tup = [imgs[:row_num,:,:,:]] + gts
        compose = torch.cat(tup,0)
        vutils.save_image(compose, fp = save_path, nrow = row_num, padding = 10)
    else:
        imgs = torchvision.transforms.Resize((h,w))(imgs)
        if prev_masks is None:
            prev_masks = torch.zeros_like(pred_masks)
        prev_masks = torchvision.transforms.Resize((h,w))(prev_masks)

        if imgs.size(1) == 1:
            imgs = imgs[:,0,:,:].unsqueeze(1).expand(b,3,h,w)
        pred_masks = pred_masks[:,0,:,:].unsqueeze(1).expand(b,3,h,w)
        gt_masks = gt_masks[:,0,:,:].unsqueeze(1).expand(b,3,h,w)
        prev_masks = prev_masks[:,0,:,:].unsqueeze(1).expand(b,3,h,w)
        channel_0 = prev_masks[:, 0, :, :].clone()  # 첫 번째 채널
        channel_1 = prev_masks[:, 1, :, :].clone()  # 두 번째 채널
        channel_2 = prev_masks[:, 2, :, :].clone()  # 세 번째 채널

        prev_masks = torch.stack([channel_0, channel_1, channel_2], dim=1)
        if points != None:
            #for i in range(b):
            if args.thd:
                p = np.round(points.cpu()/args.roi_size * args.out_size).to(dtype = torch.int)
            else:
                p = np.round(points.cpu()/args.image_size * np.array(args.out_size)).to(dtype = torch.int)
            ...
            for i in range(b):
                for j in range(len(points)):
                    ...
                    # gt_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.4
                    if pt_labels[j] == 0:
                        prev_masks[i,0,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 1.0
                        prev_masks[i,1,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 0.0
                        prev_masks[i,2,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 0.0
                    else:

                        prev_masks[i,0,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 0.0
                        prev_masks[i,1,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 1.0
                        prev_masks[i,2,p[j,0]-3:p[j,0]+3,p[j,1]-3:p[j,1]+3] = 0.0
                    # prev_masks[i,1,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.1
                    # prev_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.4

                    # imgs[i] = add_tag(imgs[i].unsqueeze(0), 'Input image')
                    # #prompt_list = ['Thinner', 'Thicker', 'Extend', 'Denoise', 'Connect']
                    # prev_masks[i] = add_tag(prev_masks[i].unsqueeze(0), 'Prev, Prompt: ' +text[i] )
                    # pred_masks[i] = add_tag(pred_masks[i].unsqueeze(0), 'Prediction mask')
                    # gt_masks[i] = add_tag(gt_masks[i].unsqueeze(0), 'GT')

                tagged_imgs.append(add_tag(imgs[i].unsqueeze(0), 'Input image'))
                tagged_prev_masks.append(add_tag(prev_masks[i].unsqueeze(0), 'Previous Mask'))
                tagged_pred_masks.append(add_tag(pred_masks[i].unsqueeze(0), 'Prediction mask'))
                tagged_gt_masks.append(add_tag(gt_masks[i].unsqueeze(0), 'GT'))


        # tup = (imgs[:row_num,:,:,:], prev_masks[:row_num,:,:,:], pred_masks[:row_num,:,:,:], gt_masks[:row_num,:,:,:])
        tup_list = []
        for b_indx in range(b):
            tup = torch.cat((tagged_imgs[b_indx],tagged_prev_masks[b_indx],tagged_pred_masks[b_indx],tagged_gt_masks[b_indx]), 0)
            tup_list.append(tup)
        
        # compose = torch.cat((imgs[:row_num,:,:,:],pred_disc[:row_num,:,:,:], pred_cup[:row_num,:,:,:], gt_disc[:row_num,:,:,:], gt_cup[:row_num,:,:,:]),0)
        compose = torch.cat(tup_list,0)
        vutils.save_image(compose, fp = save_path, nrow = row_num, padding = 10)
...
def vis_image_pred_mask(imgs, gt_masks, pred_masks, prev_masks,save_path, reverse = False, points = None, text = None, pt_labels = None):
    ...
    # if torch.max(prev_masks) > 1 or torch.min(prev_masks) < 0:
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        if points != None:
            #for i in range(b):
            if args.thd:
                p = np.round(points[0].cpu()/args.roi_size * args.out_size).to(dtype = torch.int)
            else:
                p = np.round(points[0].cpu()/args.image_size * np.array(args.out_size)).to(dtype = torch.int)
            ...
            for i in range(b):
                ...
                tagged_imgs.append(add_tag(imgs[i].unsqueeze(0), 'Input image'))
                tagged_prev_masks.append(add_tag(prev_masks[i].unsqueeze(0), 'Previous Mask'))
                #tagged_prev_masks.append(add_tag(prev_masks[i].unsqueeze(0), 'Prev'))
                tagged_pred_masks.append(add_tag(pred_masks[i].unsqueeze(0), 'Prediction mask'))
                tagged_gt_masks.append(add_tag(gt_masks[i].unsqueeze(0), 'GT'))
...
def sam_vis_image(imgs, pred_masks, gt_masks, save_path, pt_label, reverse = False, points = None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        if points != None:
            for i in range(b):
                if args.thd:
                    p = np.round(points.cpu()/args.roi_size * out_size).to(dtype = torch.int)
                else:
                    p = np.round(points.cpu()/args.image_size * out_size).to(dtype = torch.int)
                    #p = np.round(points.cpu()/(args.image_size //args.out_size)).to(dtype = torch.int)
                # gt_masks[i,:,points[i,0]-5:points[i,0]+5,points[i,1]-5:points[i,1]+5] = torch.Tensor([255, 0, 0]).to(dtype = torch.float32, device = torch.device('cuda:' + str(dev)))
                # gt_masks[i,0,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.5
                # gt_masks[i,1,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.1
                # gt_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.4
                if pt_label == 0:
                    pred_masks[i,0,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 1.0
                    pred_masks[i,1,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.0
                    pred_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.0
                else:
                    pred_masks[i,0,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.0
                    pred_masks[i,1,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 1.0
                    pred_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.0

        imgs = add_tag(imgs, 'Input image')
        pred_masks = add_tag(pred_masks, 'Prediction mask')
        gt_masks = add_tag(gt_masks, 'GT')
    ...
    return    
def vis_image(imgs, pred_masks, gt_masks, prev_masks, save_path, pt_label, reverse = False, points = None, text = None):
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        if points != None:
            for i in range(b):
                ...
                # gt_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.4
                if pt_label == 0:
                    prev_masks[i,0,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 1.0
                    prev_masks[i,1,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.0
                    prev_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.0
                else:

                    prev_masks[i,0,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.0
                    prev_masks[i,1,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 1.0
                    prev_masks[i,2,p[i,0]-3:p[i,0]+3,p[i,1]-3:p[i,1]+3] = 0.0
        if text is not None:
            imgs = add_tag(imgs, 'Input image')
            prev_masks = add_tag(prev_masks, 'Prompt: ' + text[0] )
            pred_masks = add_tag(pred_masks, 'Prediction mask')
            gt_masks = add_tag(gt_masks, 'GT')
...
def add_tag(image, tag='nodefined', tag_h=40):
    if image.requires_grad:
        image = image[0].permute(1, 2, 0).detach().cpu().numpy().astype(np.float32)  # PyTorch 텐서를 NumPy 배열로 변환
    else:
        image = image[0].permute(1, 2, 0).cpu().numpy().astype(np.float32)
    H, W = image.shape[0], image.shape[1]
    tag_blanc = np.ones((tag_h, W, 3)).astype(np.float32) * 255
    cv2.putText(tag_blanc, tag, (10, 20), cv2.FONT_HERSHEY_COMPLEX, 0.4, (0, 0, 0), 1)
    image = cv2.vconcat([image, tag_blanc])
    image = torch.tensor(image).permute(2, 0, 1).float()  # 다시 PyTorch 텐서로 변환
    return image.unsqueeze(0)

def add_tag_1c_np(image, tag='nodefined', tag_h=40):
    image = image.astype(np.float32)
    H, W = image.shape[0], image.shape[1]
    tag_blanc = np.ones((tag_h, W)).astype(np.float32)
    cv2.putText(tag_blanc, tag, (10, 20), cv2.FONT_HERSHEY_COMPLEX, 0.4, (0, 0), 1)
    image = cv2.vconcat([image, tag_blanc])
    return image

def add_tag_3c_np(image, tag='nodefined', tag_h=40):
    image = image.astype(np.float32)
    H, W = image.shape[0], image.shape[1]
    tag_blanc = np.ones((tag_h, W, 3)).astype(np.float32)
    cv2.putText(tag_blanc, tag, (10, 20), cv2.FONT_HERSHEY_COMPLEX, 0.4, (0, 0, 0), 1)
    image = cv2.vconcat([image, tag_blanc])
    return image
